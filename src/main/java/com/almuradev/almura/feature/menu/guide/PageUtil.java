...
public class PageUtil {
    ...
    public static void loadAll() {
        if (FMLCommonHandler.instance().getEffectiveSide().isServer()) {
            PageRegistry.clear();
            PageUtil.loadPages(PageUtil.PATH_PAGES, FILTER_YAML_FILES_ONLY);
            Almura.instance.logger.info("Loaded [" + PageRegistry.getAll().size() + "] page(s).");
        }
    }
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(path, filter)) {
            for (Path p : stream) {
                ...
                try {
                    ...
                    Almura.instance.logger.warn("Failed parsing file [" + p.getFileName() + "] due to invalid date format. Should be MM/dd/yyyy");
                }
            }
        } catch (IOException e) {
